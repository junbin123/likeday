...
//       31556925974.7 * (year - minYear) + sTermInfo[n] * 60000 + Date.UTC(minYear, 0, 6, 2, 5)
//     )
...
function getLunar24Days (year, month, day) {
  if (_checkYear(year) && _checkMonth(month) && _checkDay(day)) {
    ...
    for (let i = 1; i <= 24; i++) {
      ...
      if (
        getSolarDayNumber(newDate.getFullYear(), newDate.getMonth() + 1, newDate.getDate()) ===
...
export const yearList = Array(201)
  .fill(1900)
...
const dayList = (year, month) => {
  const count = getSolarMonthDaysCount(year, month)
  return Array(count)
    .fill(1)
    .map((item, index) => item + index)
}

// 根据阳历日期（年月），获取该月每一天的农历日期和阳历日期信息
export const getCalendarList = (year, month) => {
  const weekArr = ['日', '一', '二', '三', '四', '五', '六'] // 获取星期
  const firstWeek = new Date(year, month - 1, 1).getDay()
  const res = dayList(year, month).map((day, index) => {
    const week = (firstWeek + index) % 7
...
// 获取星期
export const getWeek = (year = 2020, month = 1, day = 1) => {
  const weekArr = ['日', '一', '二', '三', '四', '五', '六']
  const value = new Date(year, month - 1, day).getDay()
  return {
    name: `星期${weekArr[value]}`,
    value
  }
}
